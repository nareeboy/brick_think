 ...
 *  - custom  → a per-preset family name "custom-<id>-heading|body"; the bytes are
 *    not needed here (registerBrandFonts fetches them) — only the family name.
 */
export async function resolveBranding(
  ...
  const { data, error } = await svc
    .from('brand_profiles')
    .select(
      'id, owner_id, display_name, footer_contact, brand_colour, accent_colour, logo_path, heading_font, body_font',
    )
  ...
  return {
    ...
    logoDataUri,
    headingFamily: familyForChoice(row.heading_font, row.id, 'heading'),
    bodyFamily: familyForChoice(row.body_font, row.id, 'body'),
  };
}
