...
The app's IA is a single hierarchy with one cross-cutting index. Context comes from the URL on each page — there is no global "active org" state on the profile, no header switcher. The header has three top-level nav links — `My Designs`, `Organisations`, and `Scenarios`, in that order — plus a user block on the right: the user's avatar + display name (purely visual, not a click target), an icon-only cog linking to [`/app/account`](app/account/page.tsx), and the Sign-out form. The cog is the canonical entry point to per-account settings; the avatar and name reinforce identity but never duplicate navigation. `My Designs` sits in the leftmost position because it is the app's home destination — the BrickThink logo also routes to `/app/my-designs`, so the home link's position stays consistent whether the user clicks the brand or the first nav item.
...
- **`/app/account`** ([app/account/page.tsx](app/account/page.tsx)) is a stacked settings hub — profile form ([AccountForm](app/account/AccountForm.tsx)) with avatar control row + display name, the replay walkthrough card ([ReplayWalkthroughCard](app/account/ReplayWalkthroughCard.tsx)), the accessibility preferences card ([A11yPreferencesCard](app/account/A11yPreferencesCard.tsx) — toggles `profiles.a11y_preferences.colourblindMode` via [`updateA11yPreferencesAction`](app/account/actions.ts); the toggle drives [BrickPatternOverlay](../../components/builder/BrickPatternOverlay.tsx) rendering on every placed brick), the [IntegrationsCard](app/account/IntegrationsCard.tsx) (save / test / remove the per-user Anthropic API key used by session-report generation — server actions in [integrations-actions.ts](app/account/integrations-actions.ts); see "Session report" below and [supabase/CLAUDE.md "BYO Anthropic key"](../../supabase/CLAUDE.md) for the storage gotchas), the open-source contribution card ([ContributionCard](app/account/ContributionCard.tsx) linking to the GitHub repo and issues tracker), then a danger zone with delete-account ([DangerZone](app/account/DangerZone.tsx) → `deleteAccountAction` in [actions.ts](app/account/actions.ts), with pre-flight checks in [../../lib/account/delete.ts](../../lib/account/delete.ts)). BrickThink is open source (Apache 2.0); the prior Stripe/billing scaffolding (`BillingCard`, `/api/billing/*`, `/api/stripe/webhook`, `lib/billing/*`, `plans` / `stripe_customers` / `stripe_subscriptions` tables) was removed in the open-source flip — don't reintroduce it without an explicit decision to monetise. Delete-account refuses while the user is sole owner of an org with other members, auto-deletes sole-owner empty orgs, sweeps both the `model-thumbnails/${userId}/*` prefix and `avatars/${userId}/avatar.png`, then calls `auth.admin.deleteUser`. The flow only works because [supabase/migrations/20260516120000_profile_fk_set_null.sql](../../supabase/migrations/20260516120000_profile_fk_set_null.sql) flipped `model_versions.created_by` and `sessions.facilitator_id` to `ON DELETE SET NULL` (the NO-ACTION FKs blocked the auth delete before).

## Admin CMS — articles

Site-admin-only blogging surface. Reached via the admin area at `/app/admin` ([AdminSideNav](app/admin/AdminSideNav.tsx) lists `Articles` → `/app/admin/cms/articles`); the whole `/app/admin` subtree is gated — [admin/layout.tsx](app/admin/layout.tsx) `redirect('/app/my-designs')` for anyone whose `profiles.is_site_admin` is not true. The `is_site_admin` flag, RLS (`public read of published` + site-admin full access), the `articles` table, and the public `article-covers` Storage bucket all live in [../../supabase/migrations/20260522100000_articles_and_site_admin.sql](../../supabase/migrations/20260522100000_articles_and_site_admin.sql).

- **Routes.** List/dashboard at [app/admin/cms/articles/page.tsx](app/admin/cms/articles/page.tsx); create at [new/page.tsx](app/admin/cms/articles/new/page.tsx); edit at [\[id\]/page.tsx](app/admin/cms/articles/[id]/page.tsx). All three render the shared [ArticleEditor](app/admin/cms/articles/ArticleEditor.tsx) client component (`mode='new' | 'edit'`).
...
- **Facilitator branch** (every user without an explicit `'participant'` role): [WelcomeModal](../../components/onboarding/WelcomeModal.tsx) on first `/app/my-designs` visit → [FacilitatorChecklist](../../components/onboarding/FacilitatorChecklist.tsx) ("Three steps to your first session") → action spotlights on the relevant pages → [SpotlightTour](../../components/onboarding/SpotlightTour.tsx) (3 steps, custom SVG cut-out overlay, no library) on first session-page visit.
...
### Checklist follows the create-first-session funnel

The `FacilitatorChecklist` is pinned above the header on **every page in the funnel**, not just My Designs, so the user never loses their progress: `/app/my-designs`, `/app/orgs` (step 1 — create org), `/app/orgs/[id]` (step 2 — create session), and `/app/sessions/[id]` (step 3 — start your first model). Its three done-states + "next-click" targets come from one shared server helper, [`computeFacilitatorChecklistProgress`](../../lib/onboarding/facilitatorProgress.ts) (orgs / sessions / owned session-scoped designs). Pages that already loaded the user's orgs (My Designs, orgs list) pass `orgIds`/`firstOrgId` in to skip a duplicate membership query; the org-detail and session pages override the relevant "next" target to the context the user is already in (`firstOrgId` → current org, `firstSessionId` → current session). The checklist deliberately does **not** render on immersive pages like the design builder. Visibility/dismissal is still the shared localStorage flags, so dismissing or completing on any page carries across all of them.

### Action spotlights (checklist steps 2 & 3)

Steps 2 and 3 don't just deep-link — their `ChecklistRow` `<Link>`s carry an `?onboarding=` param (and `scroll={false}` so navigation doesn't yank to the top and fight the spotlight) that fires a focused spotlight on the destination:

- **[CreateSessionSpotlight](../../components/onboarding/CreateSessionSpotlight.tsx)** (`?onboarding=create-session`, rendered on `/app/orgs/[id]`) cuts out the **Create session** button (`data-tour-id="create-session-button"` on [CreateSessionButton](app/orgs/[id]/sessions/CreateSessionButton.tsx)).
- **[StartModelSpotlight](../../components/onboarding/StartModelSpotlight.tsx)** (`?onboarding=start-model`, rendered on `/app/sessions/[id]`) is a 2-step sequence over the first stage's **Start** button (`data-tour-id="stage-timer-start"`) then **Start your model** (`data-tour-id="start-model-button"`).

Both deep-link from anywhere (link points at the user's first org/session) **and** work same-page (the org/session pages override `firstOrgId`/`firstSessionId` to the current context, so it's a same-page client nav that flips the param). They read the param via `useSearchParams`, so they're wrapped in `<Suspense>`. The overlay is `pointer-events-none` at `z-30` so the highlighted button stays clickable and any dialog it opens (`ModalBackdrop`, `z-40`) sits above it; clicking the target advances/dismisses. Esc/Skip dismiss; the param is stripped (`router.replace`) on finish so a refresh doesn't re-fire. When `?onboarding=start-model` is present the session page passes `suppressed` to `SpotlightTour` (read server-side from `searchParams`) so the auto-tour and the action spotlight never stack.

### Spotlight rect tracking — `useSpotlightRect`

Every spotlight (the tour and both action spotlights) must keep its SVG cut-out glued to a moving target: the `FacilitatorChecklist` renders **after hydration** (it's gated on `hydrated`) and mounts above the header, pushing targets down — a layout shift that fires **no** scroll/resize event. The shared hook [`useSpotlightRect`](../../components/onboarding/useSpotlightRect.ts) tracks the target with a per-frame `requestAnimationFrame` re-measure (re-rendering only when the box actually moves, so it settles to a no-op) and centres the target via `scrollIntoView` on activation so below-the-fold buttons (e.g. the stage Start button) are brought into view. `SpotlightTour` carries its own equivalent inline loop (its multi-step silent-skip makes a clean hook share awkward); `StartModelSpotlight` uses the hook. **`CreateSessionSpotlight` still has its own inline copy — a known follow-up is to fold it onto `useSpotlightRect`.**

Tour targets are `data-tour-id` attributes on the session page: `session-header` (the `<header>`), `first-stage-card` (first `<li>` in [SessionStages](app/sessions/[id]/SessionStages.tsx)), `stage-meta-pencil` (first stage's editor button — `isTourTarget` prop on [StageMetaEditor](app/sessions/[id]/StageMetaEditor.tsx)). `stage-timer-start` and `start-model-button` are also set on the **first** stage only (via `isFirst` threaded through `ModelAction`/[StartModelButton](app/sessions/[id]/StartModelButton.tsx) and `StageTimerControls` in `SessionStages`). The tour silent-skips any step whose target isn't in the DOM, so non-managers (no pencil) collapse it to 2 steps automatically.
...
**Page-level role pills are a deliberate exception** to the card-chip shape. The session-detail role indicator ([SessionRoleChip](app/sessions/[id]/SessionRoleChip.tsx)) sits above the breadcrumb as an eyebrow-position role announcement, not inline metadata. It uses a larger `rounded-full px-3 py-1 text-[11px] tracking-[0.22em]` shape with a saturated fill: `bg-violet-600` + white text + `bg-violet-200` dot for **Facilitator**, `bg-teal-100` + `text-teal-900` + `bg-teal-600` dot for **Participant**. Don't reach for these treatments on card chips — the size and saturation are calibrated for a single page-level statement, not a grid.
...
After a session ends, the facilitator can export a branded PDF on `/app/sessions/<id>`. Schema, encryption, and Storage details live in [supabase/CLAUDE.md "Session reports" + "BYO Anthropic key"](../../supabase/CLAUDE.md); this section covers the UI / wiring only.

**Trigger surface.** [GenerateReportButton](app/sessions/[id]/GenerateReportButton.tsx) renders in the session-page header for `canManageSession` viewers when `session.status === 'completed'`. Hides on non-completed sessions (the state machine guarantees that), and hides for non-facilitator/non-admin viewers. Button surface is `bg-zinc-900 text-white` (matches the marketing primary-CTA convention) — deliberately not `bg-brand` because the report itself is dark-zinc-on-cream editorial, not a chromatic brand surface.

**Lifecycle.** Three button states drive off the `session_reports` row:

1. No row → `Generate report` (primary). Clicking calls `generateSessionReport` ([report-actions.ts](app/sessions/report-actions.ts)) — full pipeline (collect → Claude → render → upload → sign URL). 10–90s typical.
2. Row with `generation_status='succeeded'` → `Download PDF` (link, opens signed URL in a new tab) + `Regenerate` (secondary).
3. Row with `generation_status='failed'` → button reverts to `Generate report`; the page-server-rendered `initialError` flashes the failure reason in red until the user clicks.

**Hydration.** [page.tsx](app/sessions/[id]/page.tsx) calls `getLatestSessionReport(session.id)` once on render (only when `canManageSession && status === 'completed'`) and passes the result into the button. The action signs a fresh 1h URL on every page load — don't cache `pdfUrl` past a session; signed URLs expire.

**Error-code surface.** `GenerateReportResult.code` is intentionally exhaustive so the UI's `messageForCode` switch never falls through to a generic "something went wrong". Important distinctions:

- `no_claude_key` — user has no `user_integrations` row. UI surfaces a deep-link to `/app/account`.
- `decrypt_failed` — user has a row but the ciphertext can't be decrypted (almost always: `BRICKTHINK_ENCRYPTION_KEY` rotated since the row was written). UI tells them to re-paste; don't masquerade as "no key".
- `no_models` — completed session with zero participant canvases. UI: "This session has no models to include."
...
- **Header** carries a `RosterButton` (rendered with the label `Invite Members (N)` where N is the live participant count) for facilitators only, opening the modal. Visible when `session.join_code IS NOT NULL`.
- **Page-level role pill** sits above the breadcrumb on the session detail page via [SessionRoleChip](app/sessions/[id]/SessionRoleChip.tsx) — saturated violet for Facilitator, light teal for Participant. See the "Page-level role pills are a deliberate exception" note under [Badge pill convention](#badge-pill-convention) for the shape spec. Adjacent to the header on the right is the `Go to my canvas` CTA (participants only, linked to their model for the current stage).
